...
|`move(characters)`|creates the `TL abs(<characters>)` or `TR<characters>` edit descriptor that moves `<characters>` characters from the current position. `characters` must not be zero.|
...
| `format(format_items[, separator])`                    |creates a format specification as characters.<br>The separator is placed before **data and character string edit descriptors** when `separator` is passed. `separator` must not be `"`|
| `repeat(format_items[, separator][, repeat_count])`    |creates a repeated/unlimited format item.<br>The separator is placed before **data edit descriptors** when `separator` is passed.<br>`separator` must not be `"`|
