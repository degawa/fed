...
contains
    ...
    pure function construct_char_edit_descriptor() result(new_char_edit_descriptor)
        implicit none
        type(char_edit_descriptor_type) :: new_char_edit_descriptor
            !! 生成されるchar_edit_descriptor_typeインスタンス

        call new_char_edit_descriptor%set(char_edit_descriptor_symbol)
    end function construct_char_edit_descriptor

    !>char_edit_descriptor_typeインスタンスを生成して返す．
    !>欄幅には`width`が用いられる．
    pure function construct_char_edit_descriptor_w_width(width) result(new_char_edit_descriptor)
        use :: stdlib_strings
        implicit none
        ...
        if (w <= 0) then
            new_char_edit_descriptor = char()
            return
        end if

        call new_char_edit_descriptor%set(char_edit_descriptor_symbol//to_string(w))
